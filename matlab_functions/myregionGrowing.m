function [P, J] = regionGrowing(cIM, initPos, thresVal, maxDist, tfMean, tfFillHoles, tfSimplify, regVal,thresregVal)
...
% initial pixel value
if nargin < 8 || ~exist('regVal', 'var') || isempty(regVal)
    regVal = double(cIM(initPos(1), initPos(2), initPos(3)));
end

% initial pixel value
if nargin < 9 || ~exist('thresregVal', 'var') || isempty(thresregVal)
    thresregVal = regVal;
end
...
if regVal > thresregVal
    display('Initial value invalid, seed ignored.');   
else
